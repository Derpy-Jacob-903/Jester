...
public class PowerUp() : JesterCard(1, CardType.Skill,
    CardRarity.Uncommon, TargetType.Self)
{
    ...
    protected override async Task OnPlay(
        ...
        CardPlay play)
    {
        IReadOnlyList<CardPileAddResult> combat = await CardPileCmd.AddGeneratedCardsToCombat(CardFactory.GetDistinctForCombat(this.Owner, this.Owner.Character.CardPool.GetUnlockedCards(this.Owner.UnlockState, this.Owner.RunState.CardMultiplayerConstraint).Where<CardModel>((Func<CardModel, bool>) (c => c.Type == CardType.Power && !c.Keywords.Contains(CardKeyword.Unplayable))), this.ResolveEnergyXValue(), this.Owner.RunState.Rng.CombatCardGeneration), PileType.Hand, Owner);
